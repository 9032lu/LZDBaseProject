...
/** 生成条码 */
+ (UIImage *)SG_generateWithDefaultBarCodeData:(NSString *)data imageViewWidth:(CGSize)imageViewWidth {
    // 1、创建滤镜对象
    ...
    NSString *info = data;
    // 将字符串转换成
    NSData *infoData = [info dataUsingEncoding:NSASCIIStringEncoding];
    ...
    return [SGQRCodeTool createBarCodeInterpolatedUIImageFormCIImage:outputImage withSize:imageViewWidth];
}

+ (UIImage *)createBarCodeInterpolatedUIImageFormCIImage:(CIImage *)image withSize:(CGSize)size {
    if (image) {
        CGRect extent = CGRectIntegral(image.extent);
        CGFloat csaleWidth = size.width/CGRectGetWidth(extent);
        CGFloat csaleHeight = size.height/CGRectGetHeight(extent);
        
        // 1.创建bitmap;
        size_t width = CGRectGetWidth(extent) * csaleWidth;
        size_t height = CGRectGetHeight(extent) * csaleHeight;
        CGColorSpaceRef cs = CGColorSpaceCreateDeviceGray();
        CGContextRef bitmapRef = CGBitmapContextCreate(nil, width, height, 8, 0, cs, (CGBitmapInfo)kCGImageAlphaNone);
        CIContext *context = [CIContext contextWithOptions:nil];
        CGImageRef bitmapImage = [context createCGImage:image fromRect:extent];
        CGContextSetInterpolationQuality(bitmapRef, kCGInterpolationNone);
        CGContextScaleCTM(bitmapRef, csaleWidth, csaleHeight);
        CGContextDrawImage(bitmapRef, extent, bitmapImage);
...
+ (void)SG_scanningQRCodeOutsideVC:(UIViewController *)outsideVC session:(AVCaptureSession *)session previewLayer:(AVCaptureVideoPreviewLayer *)previewLayer {
    ...
    output.metadataObjectTypes = @[AVMetadataObjectTypeCode39Code,AVMetadataObjectTypeCode39Mod43Code,AVMetadataObjectTypeEAN13Code,AVMetadataObjectTypeEAN8Code,AVMetadataObjectTypeCode93Code,AVMetadataObjectTypeCode128Code,AVMetadataObjectTypePDF417Code,AVMetadataObjectTypeQRCode,AVMetadataObjectTypeAztecCode,AVMetadataObjectTypeInterleaved2of5Code,AVMetadataObjectTypeITF14Code,AVMetadataObjectTypeDataMatrixCode];
