...
#define kGloomyBlackColor  [UIColor colorWithRed:0 green:0 blue:0 alpha:1.0]
...
/* 默认超时时间，30s后自动去除提示框 */
static NSTimeInterval const interval = 8.0f;
...
+(void)customView {
    ...
    gloomyView.backgroundColor = [UIColor blackColor];
...
+ (void) showBriefAlert:(NSString *) message inView:(UIView *) view{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([message isEqualToString:@"<null>"]|| [message isKindOfClass:[NSNull class]]) {
            hud.label.text = @"";
        }else{
        ...
        hud.label.font = BMNormalFont;
        hud.margin = 12.f;
        if (hud.width >= ScreenWidth/2) {
            
            hud.label.numberOfLines = 2;
            
        }
        [hud setOffset:CGPointMake(0, ScreenHeight / 3)];
